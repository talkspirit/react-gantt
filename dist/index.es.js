...
import { locate as Le, locateID as Ie, locateAttr as Mt, dateToString as Oe, locale as Fe } from "@svar-ui/lib-dom";
...
const lt = 5;
function Xt(t, s) {
  let r, n, o, N, h, f, x, E, m;
  function P(a) {
    N = a.clientX, h = a.clientY, f = {
      ...Bt(r, t, a),
      y: s.getTask(o).$y
  ...
  function T(a) {
    r = Le(a), ot(r) && (o = ze(r), m = setTimeout(() => {
      E = !0, s && s.touchStart && s.touchStart(), P(a.touches[0]);
  ...
  function L(a) {
    t.removeEventListener("mousemove", S), t.removeEventListener("touchmove", G), document.body.removeEventListener("mouseup", C), document.body.removeEventListener("touchend", I), document.body.style.userSelect = "", a && (t.removeEventListener("mousedown", V), t.removeEventListener("touchstart", T));
  }
  function _(a) {
    ...
    if (!n) {
      if (Math.abs(B) < lt && Math.abs(O) < lt || s && s.start && s.start({ id: o, e: a }) === !1)
        return;
      n = r.cloneNode(!0), n.style.pointerEvents = "none", n.classList.add("wx-reorder-task"), n.style.position = "absolute", n.style.left = f.left + "px", n.style.top = f.top + "px", r.style.visibility = "hidden", r.parentNode.insertBefore(n, r);
    }
    if (n) {
      const J = Math.round(Math.max(0, f.top + O));
      if (s && s.move && s.move({ id: o, top: J, detail: x }) === !1)
        return;
      const j = s.getTask(o), Q = j.$y;
      if (!f.start && f.y == Q) return H();
      f.start = !0, f.y = j.$y - 4, n.style.top = J + "px";
      const X = document.elementFromPoint(
        a.clientX,
        a.clientY
      ), z = Le(X);
      if (z && z !== r) {
        const U = ze(z), R = z.getBoundingClientRect(), i = R.top + R.height / 2, p = a.clientY + f.db > i && z.nextElementSibling !== r, W = a.clientY - f.dt < i && z.previousElementSibling !== r;
  ...
  function H() {
    r && (r.style.visibility = ""), n && (n.parentNode.removeChild(n), s && s.end && s.end({ id: o, top: f.top })), o = r = n = f = x = null, L();
  }
...
function jt(t) {
  const {
    ...
    columnWidth: N = 0,
    onTableAPIChange: h
  } = t, [f, x] = Ye(N), [E, m] = re(), P = Ce(Ee.i18n), T = b(() => P.getGroup("gantt"), [P]), c = Ce(De), V = A(c, "scrollTop"), L = A(c, "cellHeight"), _ = A(c, "_scrollTask"), S = A(c, "_selected"), G = A(c, "area"), I = A(c, "_tasks"), C = A(c, "_scales"), H = A(c, "columns"), a = A(c, "_sort"), B = A(c, "calendar"), O = A(c, "durationUnit"), J = A(c, "splitTasks"), [j, Q] = re(null), X = b(() => !I || !G ? [] : I.slice(G.start, G.end), [I, G]), z = k(
    (l, g) => {
      if (g === "add-task")
        c.exec(g, {
          target: l,
  ...
  const F = ee(null), de = k(
    ...
    [c]
  ), ue = b(() => G?.from ?? 0, [G]), fe = b(() => C?.height ?? 0, [C]), le = b(() => !r && o !== "grid" ? (f ?? 0) > (n ?? 0) : (f ?? 0) > (p ?? 0), [r, o, f, n, p]), v = b(() => {
    const l = {};
    ...
    if (!$e) {
      ...
      if (te.some((u) => u.$width)) {
        let u = 0;
        $ = te.reduce((y, M) => (M.hidden || (u += M.width, y += M.$width || M.width), y), 0), u > $ && $ > g && (e = !0);
      }
      if (e || $ < g) {
        let u = 1;
  ...
  Z(() => {
    ...
    g && g.row !== l && R.current && R.current.contains(document.activeElement) && E.exec("focus-cell", {
      row: l,
      column: g.column
    });
  }, [_, E]);
  ...
  const We = k((l) => {
    m(l), l.intercept("hotkey", (g) => ae.current.handleHotkey(g)), l.intercept("scroll", () => !1), l.intercept("select-row", () => !1), l.intercept("sort-rows", (g) => {
      ...
      const { id: $, column: e, value: u } = g, y = ae.current.tasks.find((M) => M.id === $);
      if (y) {
        const M = { ...y };
        let q = u;
...
function qt({ borders: t = "" }) {
  ...
  const f = {
    width: "100%",
    height: "100%",
    background: r != null && n != null ? `url(${Lt(r, n, N, t)})` : void 0,
    position: "absolute"
  };
  return /* @__PURE__ */ d("div", { ref: o, style: f });
}
function Qt({ onSelectLink: t, selectedLink: s, readonly: r }) {
  const n = Ce(De), o = A(n, "_links"), N = A(n, "criticalPath"), h = ee(null), f = k(
    (x) => {
  ...
  }, [r, s, f]), /* @__PURE__ */ we("svg", { className: "wx-dkx3NwEn wx-links", children: [
    (o || []).map((x) => {
      const E = "wx-dkx3NwEn wx-line" + (N && x.$critical ? " wx-critical" : "") + (r ? "" : " wx-line-selectable");
      return /* @__PURE__ */ d(
        "polyline",
        {
          className: E,
...
function Ut(t) {
  ...
  function o(N) {
    ...
    const h = s.duration * s.progress / 100, f = s.segments;
    let x = 0, E = 0, m = null;
    do {
      const P = f[E];
      E === N && (x > h ? m = 0 : m = Math.min((h - x) / P.duration, 1) * 100), x += P.duration, E++;
...
function Zt(t) {
  const { readonly: s, taskTemplate: r } = t, n = Ce(De), [o, N] = je(n, "_tasks"), [h, f] = je(n, "_links"), x = A(n, "area"), E = A(n, "_scales"), m = A(n, "taskTypes"), P = A(n, "baselines"), T = A(n, "_selected"), c = A(n, "_scrollTask"), V = A(n, "criticalPath"), L = A(n, "tasks"), _ = A(n, "schedule"), S = A(n, "splitTasks"), G = A(n, "summary"), I = b(() => {
    ...
    ...h.find((e) => e.id === Q)
  }, [Q, f]), [U, R] = re(void 0), i = ee(null), [p, W] = re(0), D = ee(null), Y = b(() => {
    const e = D.current;
  ...
  const de = k(() => {
    ...
    (e, u, y) => {
      if (u.target.classList.contains("wx-line") || (y || (y = n.getTask(ze(e))), y.type === "milestone" || y.type === "summary")) return "";
      const M = Le(u, "data-segment");
      M && (e = M);
    ...
    (e, u) => {
      const { clientX: y } = u, M = ze(e), q = n.getTask(M), ge = u.target.classList;
      if (!u.target.closest(".wx-delete-button") && !u.target.closest("[data-interactive]") && !s) {
    ...
    if (j.current) {
      const { dx: e, id: u, marker: y, value: M } = j.current;
      ...
      const { id: e, mode: u, dx: y, l: M, w: q, start: ge, segment: pe, index: ye } = O;
      if (J(null), ge) {
        ...
        if (!Ne)
          ...
          pe && (Ae = Ae.segments[ye]), u === "move" ? (He.start = Ae.start, He.end = Ae.end) : He[u] = Ae[u], n.exec("update-task", {
            id: e,
            diff: Ne,
            task: He,
            ...pe && { segmentIndex: ye }
    ...
    (e, u) => {
      ...
      if (!s)
        if (j.current) {
          ...
          const M = Le(e);
          if (M) {
            const q = n.getTask(ze(M)), pe = Le(e, "data-segment") || M, ye = fe(pe, u, q);
  ...
  const Te = k(
    ...
    (e, u) => {
      ...
      return e === y ? !0 : !!h.find((q) => q.target == e && q.source == y && q.type === xe(M, u));
    },
    [a, f, xe]
  ), w = k(() => {
    a && B(null);
              ...
              "div",
              {
                ...
                children: [
                  ...
                  e.type !== "milestone" ? /* @__PURE__ */ we(Pe, { children: [
                    ...
                    !s && !(S && e.segments) && !(e.type == "summary" && G?.autoProgress) ? /* @__PURE__ */ d(
                      "div",
                      {
                    ...
                    r ? /* @__PURE__ */ d("div", { className: "wx-GKbcLEGA wx-content", children: /* @__PURE__ */ d(r, { data: e, api: n, onAction: l }) }) : S && e.segments ? /* @__PURE__ */ d(Ut, { task: e, type: We(e.type) }) : /* @__PURE__ */ d("div", { className: "wx-GKbcLEGA wx-content", children: e.text || "" })
  ...
  return /* @__PURE__ */ d("div", { className: "wx-ZkvhDKir wx-scale", style: { width: n.width }, children: (n?.rows || []).map((o, N) => /* @__PURE__ */ d(
    "div",
    {
      ...
      children: (o.cells || []).map((h, f) => {
        const x = s ? s(h.date, h.unit) : "", E = "wx-cell " + (h.css || "") + " " + (x || "");
        return /* @__PURE__ */ d(
          "div",
          {
...
function nn(t) {
  const {
    ...
    cellBorders: N,
    highlightTime: h
  } = t, f = Ce(De), [x, E] = je(f, "_selected"), m = A(f, "scrollTop"), P = A(f, "cellHeight"), T = A(f, "cellWidth"), c = A(f, "_scales"), V = A(f, "_markers"), L = A(f, "_scrollTask"), _ = A(f, "zoom"), [S, G] = re(), I = ee(null), C = 1 + (c?.rows?.length || 0), H = b(() => {
    const i = [];
...
function on(t) {
  const {
    ...
    highlightTime: o,
    onTableAPIChange: N
  } = t, h = Ce(De), f = A(h, "_tasks"), x = A(h, "_scales"), E = A(h, "cellHeight"), m = A(h, "columns"), P = A(h, "_scrollTask"), T = A(h, "undo"), [c, V] = re(!1);
  let [L, _] = re(0);
  const [S, G] = re(0), [I, C] = re(0), [H, a] = re(void 0), [B, O] = re("all"), J = ee(null), j = k(
    (v) => {
...
const fn = (t) => t.split("-").map((s) => s ? s.charAt(0).toUpperCase() + s.slice(1) : "").join(""), hn = [
  ...
  const Y = ee();
  Y.current = W;
  const F = b(() => new At(Pt), []), de = b(() => ({ ...qe, ...Ke }), []), ue = Ce(Ee.i18n), fe = b(() => ue ? ue.extend(de, !0) : Fe(de), [ue, de]), le = b(() => fe.getRaw().calendar, [fe]), v = b(() => {
    let ie = {
      zoom: un(I, le),
      scales: xt(x, le),
      columns: dn(E, le),
...
function Hn({ api: t = null, items: s = [] }) {
  const r = Ce(Ee.i18n), n = b(() => r || Fe(Ke), [r]), o = b(() => n.getGroup("gantt"), [n]), N = Se(t, "_selected"), h = Se(t, "undo"), f = Se(t, "history"), x = Se(t, "splitTasks"), E = ["undo", "redo"], m = b(() => {
    ...
    return m.forEach((c) => {
      ...
      if (V === "add-task")
        ...
        T.push({
          ...c,
          disabled: c.isDisabled && N.some((L) => c.isDisabled(L, t.getState()))
        });
      }
    }), T.filter((c, V) => {
      if (t && c.isHidden)
        return !N.some((L) => c.isHidden(L, t.getState()));
      if (c.comp === "separator") {
        const L = T[V + 1];
  ...
  return r ? /* @__PURE__ */ d(rt, { items: P }) : /* @__PURE__ */ d(Ee.i18n.Provider, { value: n, children: /* @__PURE__ */ d(rt, { items: P }) });
}
const In = ct(function({
  ...
  const m = ee(null), P = ee(null), T = Ce(Ee.i18n), c = b(() => T || Fe({ ...Ke, ...qe }), [T]), V = b(() => c.getGroup("gantt"), [c]), L = Se(r, "taskTypes"), _ = Se(r, "selected"), S = Se(r, "_selected"), G = Se(r, "splitTasks"), I = Se(r, "summary"), C = b(
    () => ({
      splitTasks: G,
      taskTypes: L,
      summary: I
  ...
  const O = b(() => B(), [r, s, C, V]), J = b(
    ...
    (R) => {
      const i = R.action;
      i && (ft(H, i.id) && ut(r, i.id, P.current, V), f && f(R));
    },
    [r, V, f, H]
  ), X = k(
    (R, i) => {
  ...
  if (!T && Ee.i18n?.Provider) {
    const R = Ee.i18n.Provider;
    return /* @__PURE__ */ d(R, { value: c, children: U });
  }
  return U;
});
function mn({ api: t, autoSave: s, onLinksChange: r }) {
  const o = Ce(Ee.i18n).getGroup("gantt"), N = A(t, "activeTask"), h = A(t, "_activeTask"), f = A(t, "_links"), x = A(t, "schedule"), E = A(t, "unscheduledTasks"), [m, P] = re();
  function T() {
    if (N) {
      const _ = f.filter((G) => G.target == N).map((G) => ({ link: G, task: t.getTask(G.source) })), S = f.filter((G) => G.source == N).map((G) => ({ link: G, task: t.getTask(G.target) }));
      return [
...
function xn(t) {
  const { value: s, time: r, format: n, onchange: o, onChange: N, ...h } = t, f = N ?? o;
  function x(E) {
    const m = new Date(E.value);
    m.setHours(s.getHours()), m.setMinutes(s.getMinutes()), f && f({ value: m });
  }
  return /* @__PURE__ */ we("div", { className: "wx-hFsbgDln date-time-controll", children: [
    ...
    r ? /* @__PURE__ */ d(vt, { value: s, onChange: f, format: n }) : null
  ] });
}
Ge("select", Ct);
Ge("date", xn);
...
function zn({
  ...
  const P = Ce(Ee.i18n), T = b(() => P || Fe({ ...Ke, ...qe }), [P]), c = b(() => T.getGroup("gantt"), [T]), V = T.getRaw(), L = b(() => {
...
function Wn(t) {
  const { api: s, content: r, children: n } = t, o = ee(null), N = ee(null), [h, f] = re({}), [x, E] = re(null), [m, P] = re({});
  function T(C) {
    for (; C; ) {
      if (C.getAttribute) {
  ...
  function _(C) {
    ...
    L(() => {
      ...
      let z, U;
      O === "left" ? (z = j.top + 5 - X.top, U = j.right + 5 - X.left) : (z = j.top + j.height - X.top, U = J - X.left), f(X), P({ top: z, left: U, text: a });
    });
  }
  function S(C) {

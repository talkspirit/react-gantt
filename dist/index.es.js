...
function Xt(t, n) {
  ...
  function c(a) {
    if (E || m) return a.preventDefault(), !1;
  }
  function O(a) {
    a.which === 1 &&
      ((r = Le(a)),
  ...
  function _(a) {
    ...
    if (s) {
      ...
      if (n && n.move && n.move({ id: o, top: J, detail: x }) === !1) return;
      const j = n.getTask(o),
        Q = j.$y;
      ...
      if (z && z !== r) {
        const U = ze(z),
          M = z.getBoundingClientRect(),
          i = M.top + M.height / 2,
          p = a.clientY + f.db > i && z.nextElementSibling !== r,
          W = a.clientY - f.dt < i && z.previousElementSibling !== r;
        x?.after == U || x?.before == U
          ? (x = null)
          : p
            ? (x = { id: o, after: U })
            : W && (x = { id: o, before: U });
  ...
  function G(a) {
    E
      ? (a.preventDefault(), _(a.touches[0]))
...
function jt(t) {
  const {
    ...
    J = A(c, 'splitTasks'),
    [j, Q] = re(null),
    X = b(() => (!I || !G ? [] : I.slice(G.start, G.end)), [I, G]),
    z = k(
      (l, g) => {
        if (g === 'add-task')
          c.exec(g, {
  ...
  const F = ee(null),
    ...
    te = b(() => {
      ...
      const g = l.findIndex((e) => e.id === 'text'),
        $ = l.findIndex((e) => e.id === 'add-task');
      if (
        (g !== -1 && (l[g].cell && (l[g]._cell = l[g].cell), (l[g].cell = Yt)),
        $ !== -1)
      ) {
        l[$].cell = l[$].cell || it;
        const e = l[$].header;
    ...
    be = k(() => te.some((l) => l.flexgrow && !l.hidden), []),
    $e = b(() => be(), [be, D]),
    me = b(() => {
    ...
    Ce = k(() => {
      te.filter((g) => g.flexgrow && !g.hidden).length === 1 &&
        te.forEach((g) => {
    ...
    ie = k(
      (l) => {
        if (!n) {
          const g = Ie(l),
            $ = Mt(l, 'data-col-id');
          !($ && te.find((u) => u.id == $))?.editor &&
            g &&
            c.exec('show-editor', { id: g });
        }
      },
      [c, n],
      // cols is defined later; relies on latest value at call time
    ),
    xe = b(() => (Array.isArray(S) ? S.map((l) => l.id) : []), [S]),
    he = k(() => {
  ...
  Me(),
    Z(() => {
      Me();
    }, [m, K, a, c, Ce, ke, X, B, V, J, h]);
  const We = k((l) => {
    m(l),
      l.intercept('hotkey', (g) => ae.current.handleHotkey(g)),
      l.intercept('scroll', () => !1),
      l.intercept('select-row', () => !1),
      l.intercept('sort-rows', (g) => {
        const $ = ae.current.sortVal,
          { key: e, add: u } = g,
      ...
      l.intercept('update-cell', (g) => {
        const { id: $, column: e, value: u } = g,
          y = ae.current.tasks.find((N) => N.id === $);
        if (y) {
          const N = { ...y };
          let q = u;
      ...
      if (j.current) {
        ...
        if ((J(null), ge)) {
          ...
          if (!Ne)
            ...
            let He = {},
              Ae = s.getTask(e);
            pe && (Ae = Ae.segments[ye]),
              u === 'move'
                ? ((He.start = Ae.start), (He.end = Ae.end))
                : (He[u] = Ae[u]),
              s.exec('update-task', {
                id: e,
                diff: Ne,
      ...
      (e, u) => {
        ...
        if (!n)
          if (j.current) {
            ...
            (j.current.value = Ne = Math.min(Math.max(0, Ne), 100)),
              s.exec('update-task', {
                id: ge,
                task: { progress: Ne },
                inProgress: !0,
    ...
    he = k(
      (e, u) => {
        ...
        return e === y
          ? !0
          : !!h.find(
              (q) => q.target == e && q.source == y && q.type === xe(N, u),
            );
      },
      [a, f, xe],
    ),
      ...
      I.map((e) => {
        ...
        const u =
          ...
          y =
            'wx-link wx-left' +
            (a ? ' wx-visible' : '') +
            (!a || (!he(e.id, !0) && $(e.id)) ? ' wx-target' : '') +
            (a && a.id === e.id && a.start ? ' wx-selected' : '') +
            (g(e.id) ? ' wx-critical' : ''),
          N =
            'wx-link wx-right' +
            (a ? ' wx-visible' : '') +
            (!a || (!he(e.id, !1) && $(e.id)) ? ' wx-target' : '') +
            (a && a.id === e.id && !a.start ? ' wx-selected' : '') +
            (g(e.id) ? ' wx-critical' : '');
        return /* @__PURE__ */ we(
          wt,
  ...
  const j = k(
    (i) => {
      ...
      if (F.$x + F.$w < D.scrollLeft)
        f.exec('scroll-chart', { left: F.$x - (T || 0) }),
          (D.scrollLeft = F.$x - (T || 0));
      else if (F.$x >= Y + D.scrollLeft) {
        const de = Y < F.$w ? T || 0 : F.$w;
        f.exec('scroll-chart', { left: F.$x - Y + de }),
          (D.scrollLeft = F.$x - Y + de);
      }
    },
    [f, T],
  );
...
function sn(t) {
  ...
  function O(W) {
    ...
    if (s != 'x') for (let F in Y) Y[F] = Y[F].reverse();
    return Y;
  }
  const [L, _] = re(!1),
    [S, G] = re(null),
  ...
  const V = k(
      (W) => {
        ...
        P(D);
        let Y;
        D <= x ? (Y = 'chart') : f - D <= E ? (Y = 'grid') : (Y = 'all'),
          a.current !== Y && (c(Y), (a.current = Y)),
          H.current && clearTimeout(H.current),
          (H.current = setTimeout(() => o && o(D), 100));
      },
      [f, x, E, o],
    ),
    J = k(() => {
  ...
  function z(W) {
    if (h) {
      ...
      c(D), R(D);
    } else if (T === 'grid' || T === 'chart') X(), R('all');
    else {
...
function on(t) {
      ...
      (v) => {
        O(
          (se) => (
            v !== se &&
              (v
                ? ((J.current = B), B === 'all' && V('grid'))
                : (!J.current || J.current === 'all') && V('all')),
            v
          ),
        );
      },
  ...
  const Q = b(() => {
    ...
    return (
      m.every((se) => se.width && !se.flexgrow)
        ? (v = m.reduce((se, te) => se + parseInt(te.width), 0))
        : c && B === 'chart'
  ...
  const X = b(() => (S ?? 0) - (H ?? 0), [S, H]),
    z = b(() => x.width, [x]),
    U = b(() => f.length * E, [f, E]),
    M = b(() => x.height + U + X, [x, U, X]),
    i = b(() => L + z, [L, z]),
    p = ee(null),
    W = k(() => {
  ...
  Z(() => {
    let v;
    return (
      p.current && ((v = new ResizeObserver(W)), v.observe(p.current)),
      () => {
        v && v.disconnect();
      }
    );
  ...
  const ue = k(
    (v) => {
      ...
      if (!be) return;
      const { id: me } = v,
        ke = se.findIndex((Ce) => Ce.id === me);
      if (ke > -1) {
        const Ce = $e - te.height,
          ie = ke * oe,
          xe = be.scrollTop;
        let he = null;
        ie < xe ? (he = ie) : ie + oe > xe + Ce && (he = ie - Ce + oe + Te),
          he !== null &&
            (h.exec('scroll-chart', { top: Math.max(he, 0) }),
            (D.current.scrollTop = Math.max(he, 0)));
      }
    },
    [h],
  );
...
const fn = (t) =>
  ...
  An = ct(function (
    ...
    const F = b(() => new At(Pt), []),
      de = b(() => ({ ...qe, ...Ke }), []),
      ue = ve(Ee.i18n),
      fe = b(() => (ue ? ue.extend(de, !0) : Fe(de)), [ue, de]),
      le = b(() => fe.getRaw().calendar, [fe]),
      v = b(() => {
        let ie = {
          zoom: un(I, le),
          scales: xt(x, le),
        ...
        return (
          ie.zoom &&
            (ie = {
              ...ie,
              ...It(ie.zoom, an(le, fe.getGroup('gantt')), ie.scales, O),
            }),
          ie
        );
      }, [I, x, E, f, O, le, fe]),
    ...
    const me = b(
      () => ({
        ...
        getTable: (ie) =>
          ie
            ? new Promise((xe) => setTimeout(() => xe($e.current), 1))
            : $e.current,
    ...
    const Ce = b(
      () =>
        z
          ? (ie, xe) =>
              (xe == 'day' && !z.getDayHours(ie)) ||
              (xe == 'hour' && !z.getDayHours(ie))
                ? 'wx-weekend'
                : ''
          : H,
      [z, H],
    );
    return /* @__PURE__ */ d(Ee.i18n.Provider, {
      value: fe,
  ...
  const r = ve(Ee.i18n),
    ...
    m = b(() => {
      ...
      return (
        ...
        let L = { ...O, disabled: !1 };
        return (
          (L.handler = ft(T, L.id) ? (_) => ut(t, _.id, null, o) : L.handler),
        ...
        m.forEach((c) => {
          ...
          if (O === 'add-task') T.push(c);
            ...
            T.push({
              ...c,
              disabled:
                c.isDisabled && R.some((L) => c.isDisabled(L, t.getState())),
            });
          }
        }),
        T.filter((c, O) => {
...
const In = ct(function (
  ...
  function a(M) {
    return M.map(
      (i) => (
        (i = { ...i }),
        i.text && (i.text = O(i.text)),
        i.subtext && (i.subtext = O(i.subtext)),
    ...
    j = k(
      (M, i) => {
        ...
        if (p) {
          ...
          W !== null
            ? (P.current = { id: p.id, segmentIndex: W })
            : (P.current = p.id),
            (!Array.isArray(_) || !_.includes(p.id)) &&
              r &&
              r.exec &&
    ...
    X = k(
      (M, i) => {
        ...
        if (
          W &&
          (M.isHidden &&
            (W = !p.some((D) => M.isHidden(D, r.getState(), P.current))),
          M.isDisabled)
        ) {
...
function mn({ api: t, autoSave: n, onLinksChange: r }) {
  const o = ve(Ee.i18n).getGroup('gantt'),
    R = A(t, 'activeTask'),
    h = A(t, '_activeTask'),
    f = A(t, '_links'),
    x = A(t, 'schedule'),
    E = A(t, 'unscheduledTasks'),
    [m, P] = re();
  function T() {
    if (R) {
      const _ = f
      ...
      _.data.length
        ? /* @__PURE__ */ d(
            'div',
            {
              ...
              children: /* @__PURE__ */ d(Ee.fieldId.Provider, {
                value: null,
...
function xn(t) {
  const { value: n, time: r, format: s, onchange: o, onChange: R, ...h } = t,
    f = R ?? o;
  function x(E) {
...
function zn({
  ...
  const H = A(t, '_activeTask'),
    a = A(t, 'activeTask'),
    B = A(t, 'unscheduledTasks'),
    V = A(t, 'summary'),
    J = A(t, 'links'),
    j = A(t, 'splitTasks'),
    Q = b(() => j && a?.segmentIndex, [j, a]),
    X = b(() => Q || Q === 0, [Q]),
    z = A(t, 'taskTypes'),
    U = b(
    ...
    fe = b(() => {
      ...
      if ((X && H.segments ? (w = { ...H.segments[Q] }) : (w = { ...H }), o)) {
        ...
        return (
          U.forEach(({ key: ce, comp: K }) => {
            if (K !== 'links') {
              const ae = w[ce];
              K === 'date' && ae instanceof Date
                ? (ne[ce] = L(ae))
                : K === 'slider' && ce === 'progress'
                  ? (ne[ce] = `${ae}%`)
                  : (ne[ce] = ae);
            }
          }),
  ...
  function le(w, ne) {
    return w.map((ce) => {
      const K = { ...ce };
      if (
        (ce.config && (K.config = { ...K.config }),
        K.comp === 'links' &&
          t &&
          ((K.api = t), (K.autoSave = x), (K.onLinksChange = te)),
        K.comp === 'select' && K.key === 'type')
      ) {
        const ae = K.options ?? [];
        K.options = ae.map((Me) => ({
          ...Me,
      ...
      return (
        K.comp === 'slider' &&
          K.key === 'progress' &&
          (K.labelTemplate = (ae) => `${c(K.label)} ${ae}%`),
        K.label && (K.label = c(K.label)),
        K.config?.placeholder &&
          (K.config.placeholder = c(K.config.placeholder)),
        ne &&
          (K.isDisabled && K.isDisabled(ne, t.getState())
            ? (K.disabled = !0)
            : delete K.disabled),
        K
      );
  ...
  const v = b(() => {
      ...
      return (
        (w = le(w, Y)),
        Y ? w.filter((ne) => !ne.isHidden || !ne.isHidden(Y, t.getState())) : w
      );
    }, [n, U, Y, c, t, x]),
    se = b(() => v.map((w) => w.key), [v]);
  ...
  const oe = k(() => {
    ...
    me = k(
      (w, ne, ce) => (
        B && w.type === 'summary' && (w.unscheduled = !1),
        dt(w, t.getState(), ne),
        ce || D(!1),
        w
      ),
    ...
    ke = k(
      (w) => {
        (w = {
          ...
          delete w.links,
          delete w.data,
          (se.indexOf('duration') === -1 || (w.segments && !w.duration)) &&
            delete w.duration;
        const ne = {
          id: a?.id || a,
          task: w,
    ...
    Ce = k(
      (w) => {
        ...
        if ((K && D(!0), (w.update = me({ ...ne }, ce, K)), !x)) F(w.update);
          ...
          const ae = v.find((l) => l.key === ce),
            Me = ne[ce];
          (!ae.validation || ae.validation(Me)) &&
            (!ae.required || Me) &&
            ke(w.update);
        }
      },
...
function Wn(t) {
  ...
  function T(C) {
    for (; C; ) {
      if (C.getAttribute) {
        const H = C.getAttribute('data-tooltip-id'),
  ...
  Z(() => {
    ...
    if (C && m && (m.text || r)) {
      ...
      let a = !1,
        B = m.left,
        V = m.top;
      H.right >= h.right && ((B = h.width - H.width - 5), (a = !0)),
        H.bottom >= h.bottom &&
          ((V = m.top - (H.bottom - h.bottom + 2)), (a = !0)),
        a && P((J) => J && { ...J, left: B, top: V });
    }
  }, [m, h, r]);
  const c = ee(null),
    O = 300,
  ...
  function _(C) {
    ...
    L(() => {
      ...
      let z, U;
      V === 'left'
        ? ((z = j.top + 5 - X.top), (U = j.right + 5 - X.left))
        : ((z = j.top + j.height - X.top), (U = J - X.left)),
        f(X),
        P({ top: z, left: U, text: a });
    });
  }
  function S(C) {
